...
static const unsigned char *cc_md5_body(cc_md5_t *ctx, const unsigned char *data, unsigned long long size);
...
void cc_md5_update(cc_md5_t *ctx, const void *data, unsigned long long size) {
  ...
  used = (unsigned long long)(ctx->bytes & 0x3f);
  ctx->bytes += size;
  ...
  if (used) {
    ...
    size -= free;
    (void)cc_md5_body(ctx, ctx->buffer, 64);
  }

  if (size >= 64) {
    data = cc_md5_body(ctx, data, size & ~(unsigned long long)0x3f);
    size &= 0x3f;
  }
...
#if (cc_HAVE_LITTLE_ENDIAN && cc_HAVE_NONALIGNED)

#define SET(n) (*(unsigned int *)&p[n * 4])
#define GET(n) (*(unsigned int *)&p[n * 4])

#else

#define SET(n)                                                                                                  \
  (block[n] = (unsigned int)p[n * 4] | ((unsigned int)p[n * 4 + 1] << 8) | ((unsigned int)p[n * 4 + 2] << 16) | \
              ((unsigned int)p[n * 4 + 3] << 24))
