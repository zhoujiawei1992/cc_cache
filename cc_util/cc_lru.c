...
cc_lru_cache_t *cc_lru_cache_create(size_t capacity, cc_hash_func_t *hash_func,
  ...
  cache->hash_table = cc_hash_table_create(bucket_size, hash_func, destroy_node_func);
  if (!cache->hash_table) {
    cc_free(cache);
    return NULL;
  }
...
void cc_lru_cache_insert(cc_lru_cache_t *cache, cc_lru_cache_node_t *node) {
  if (cache->size >= cache->capacity) {
    ...
    cc_hash_table_delete(cache->hash_table, last_cache_node->hash_node.key);
    cache->size--;
  }
...
void cc_lru_cache_delete(cc_lru_cache_t *cache, const char *key) {
  ...
  if (cc_hash_table_lookup(cache->hash_table, key, &hash_node) == CC_HASH_OK) {
    ...
    cc_dlist_delete(&(cache->list_head), &(node->list_node));
    cc_hash_table_delete(cache->hash_table, key);
    cache->size--;
  }
}
